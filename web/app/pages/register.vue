<template>
    <div class="flex items-center justify-center min-h-screen">
        <form @submit.prevent="handleRegister" class="border rounded-lg border-gray-300 p-6 space-y-6 w-96 text-center">
            <div>
                <label class="block font-semibold mb-1">Prénom</label>
                <input v-model="form.firstname" type="text" placeholder="Prénom" class="w-min border rounded px-2 py-2" required="true" />
            </div>

            <div>
                <label class="block font-semibold mb-1">Nom</label>
                <input v-model="form.lastname" type="text" placeholder="Nom" class="w-min border rounded px-2 py-2" required="true" />
            </div>
